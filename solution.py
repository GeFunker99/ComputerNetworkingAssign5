...
def build_packet():
    ...
    oneChecksum = 0
    header = struct.pack("bbHHh", ICMP_ECHO_REQUEST, 0, oneChecksum, ID, 1)
    data = struct.pack("d", time.time())
    # Append checksum to the header.
    oneChecksum = checksum(header + data)

    if sys.platform == 'darwin':
        oneChecksum = htons(oneChecksum) & 0xffff
    else:
        oneChecksum = htons(oneChecksum)
    ...
    for ttl in range(1, MAX_HOPS):
        for tries in range(TRIES):
            ...
            try:
                ...
                type, code, oneChecksum, packetID, sequence = struct.unpack("bbHHh", icmpHeader)
